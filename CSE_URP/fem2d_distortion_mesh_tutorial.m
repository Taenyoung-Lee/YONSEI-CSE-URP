...
% m,n에도 random number 대입

m = randi([3,10]);
n = randi([3,10]);

[c4v,v4e] = distor2dmesh(m,n); % m by n; m이 가로
...
function [c4v,v4e] = distor2dmesh(m, n)
    ...
    a = linspace(0,1,m+1);%notation: a1등은 x,y로 표현
    aa = linspace(0,1,n+1);
    a1 = repmat(a',n+1,1);
    ...
    % 조건을 만족하는 행에 대해서만 각 행에 대해 서로다른 randn행을 더하기(by 행렬연산)
    randn_mat = randn(sum(condition), 2) * (1/n) * (1/m) * (1/2) ; %size는 내 기준 적당히 작은것 선택. 1/2를 곱해주는것 가능
