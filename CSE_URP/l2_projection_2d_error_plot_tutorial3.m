...
for idx = 1:numel(n_values)
    ...
    for i = 1:size(v4e, 2)
        ...
        n_Pf = cross(v1, v2); % 법선 벡터
        A_Pf = n_Pf(1);
        B_Pf = n_Pf(2);
        C_Pf = n_Pf(3);
        D_Pf = -A_Pf * x(1) - B_Pf * y(1) - C_Pf * Pf_values(1);
        ...
        integral_value = GQintegral2DTriangle(@(x,y) f_plane_minus_Pf_plane(1)*x + f_plane_minus_Pf_plane(2)*y + f_plane_minus_Pf_plane(3), f_matrix(1,:2), f_matrix(2,:2), f_matrix(3,:2), 0, 3, 3);
