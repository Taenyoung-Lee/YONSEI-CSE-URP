...
%%% Projection 함수 정의
function [projected_function] = l2_projection_1D(x_start,x_end, f, n_values)

    n = n_values; % 행 및 열의 개수
    x_interval = linspace(x_start, x_end, n);


    % % % % n = length(x);
    h = x_interval(2) - x_interval(1);
    ...
    for i = 1:n-1
        ...
        nGQ = 4; % 가우스 적분의 점의 개수
        approx_values = GQIntegral1D(f, x_interval(i), x_interval(i+1), nGQ, 10);
    ...
    % Projection 계산
    projected_function = l2_projection_1D(x_interval, f);
end